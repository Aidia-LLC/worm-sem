...
function curry(f: Function, n?: number): Function {
  ...
  return function () {
    return curry.apply(
      undefined,
      args.concat(Array.prototype.slice.call(arguments, 0)) as unknown as [
        Function,
        number
      ]
    );
...
// function _setCanvasSizeFromImage(
//   canvas: HTMLCanvasElement,
//   image: HTMLImageElement
// ) {
//   canvas.style.width = "";
...
function toGradientMagnitude(
  ...
  const gradientMagnitudeLt = gradientMagnitudeCapped.map((value) =>
    value < lt ? 0 : value
  );
    ...
    for (var x = 1; x < width - 1; x++) {
      ...
      if (
        ...
        (gradientMagnitudeLt[index] <=
          gradientMagnitudeLt[(y - 1) * width + x] ||
          gradientMagnitudeLt[(y + 1) * width + x] >=
            gradientMagnitudeLt[index])
      )
        gradientMagnitudeLt[index] = 0;
  ...
  for (let y = 2; y < height - 2; y++) {
    for (let x = 2; x < width - 2; x++) {
      if (gradientDirection[y * width + x] == 0)
        if (
          gradientMagnitudeLt[y * width + x - 2] >
            gradientMagnitudeLt[y * width + x] ||
          gradientMagnitudeLt[y * width + x + 2] >
            gradientMagnitudeLt[y * width + x]
        )
      ...
      if (gradientDirection[y * width + x] == Math.PI / 2)
        if (
          gradientMagnitudeLt[(y - 2) * width + x] >
            gradientMagnitudeLt[y * width + x] ||
          gradientMagnitudeLt[(y + 2) * width + x] >
            gradientMagnitudeLt[y * width + x]
        )
      ...
      if (gradientDirection[y * width + x] == Math.PI / 4)
        if (
          gradientMagnitudeLt[(y + 2) * width + x - 2] >
            gradientMagnitudeLt[y * width + x] ||
          gradientMagnitudeLt[(y - 2) * width + x + 2] >
            gradientMagnitudeLt[y * width + x]
        )
  ...
  const gradientMagnitudeUt = gradientMagnitudeLt.map((value) =>
    value > ut ? 1 : value
  );
  ...
  while (nastavi) {
    ...
    for (let y = 1; y < height - 1; y++) {
      for (let x = 1; x < width - 1; x++) {
        if (
          gradientMagnitudeUt[y * width + x] <= ut &&
          gradientMagnitudeUt[y * width + x] >= lt
        ) {
    ...
    gradientMagnitudeCappedBottom = gradientMagnitudeUt.map((x) =>
      x <= ut ? 0 : x
    );
  }
...
function getTresholds(gradientMagnitude: Array<number>): {
  ut: number;
  lt: number;
} {
  let sum = 0;
...
export function convert(width: number, height: number, imageData: Uint8Array) {
  const toConvolutionForImg = toConvolution(width, height);

  const grayscale = toPixels(toGrayscale(imageData));
  ...
  return toPixels(toDenormalized(gradientMagnitude.data));
}
